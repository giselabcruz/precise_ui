...
function StoreManager() {
  ...
  const logisticsCenters = ['Centro A', 'Centro B', 'Centro C'];
  const products = [
    { name: 'Producto 1', quantity: 100, center: 'Centro A' },
    { name: 'Producto 2', quantity: 50, center: 'Centro B' },
    { name: 'Producto 3', quantity: 200, center: 'Centro C' },
  ];
  ...
  return (
    <div className="store-manager-container p-6 bg-red-50 min-h-screen">
      <h1 className="text-2xl text-red-700 font-bold mb-6">Gestión de Centros Logísticos</h1>
                ...
                <li className="flex items-center">
                  <label className="ml-2 font-medium cursor-pointer">
                    <input
                      ...
                      checked={selectedCenters.length === logisticsCenters.length}
                      onChange={() =>
                        selectedCenters.length === logisticsCenters.length
                          ? setSelectedCenters([])
                          : setSelectedCenters(logisticsCenters)
                      }
                    />
      ...
      <div className="product-list">
        <h2 className="text-xl text-red-600 font-semibold mb-4">Productos</h2>
